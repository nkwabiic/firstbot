...
import { MemoryEventBus } from './src/conversation/events/event-bus';
...
async function runE2E() {
  ...
  eventBus.subscribe(event => {
    ...
    if (event.event === 'AIRequested') {
       if (event.state !== 'REVIEW' && event.state !== 'DONE') {
          // This tells us the prompt that would be sent. 
       }
    }
  });
