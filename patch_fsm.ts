...
export class FSMEngine {
  ...
  public async processMessage(
    ...
    if (meta.isReviewMode) {
      await this.handleReviewMode(message, conversation, user, lang, cv, meta);
      return;
    }
  ...
  private async handleIntent(
    ...
    if (intentResult.intent === 'answer' || intentResult.intent === 'correction') {
      ...
      if (meta.currentFieldId && meta.currentFieldId !== 'ADD_ANOTHER' && meta.currentFieldId !== 'CONFIRM_SKIP' && meta.currentFieldId !== 'DONE') {
          const validation = this.validateFieldLocally(meta.currentFieldId, message);
          ...
          const finalVal = validation.formattedValue || message.trim();
      ...
      } else if (intentResult.extractedData) {
          await this.applyExtractedData(intentResult.extractedData, cv);
      }
